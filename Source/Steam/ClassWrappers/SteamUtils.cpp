...
#pragma region SteamUtils007

// Return the number of seconds since
uint32 SteamUtils007::GetSecondsSinceAppActive()
{
	return RedactedUtils::GetSecondsSinceAppActive();
}
uint32 SteamUtils007::GetSecondsSinceComputerActive()
{
	return RedactedUtils::GetSecondsSinceComputerActive();
}

// Each universe is a self-contained Steam instance.
EUniverse SteamUtils007::GetConnectedUniverse()
{
	return RedactedUtils::GetConnectedUniverse();
}

// Steam servertime, UNIX time.
uint32 SteamUtils007::GetServerRealTime()
{
	return RedactedUtils::GetConnectedUniverse();
}

// Return the two digit ISO 3166-1-alpha-2 country code.
const char *SteamUtils007::GetIPCountry()
{
	return RedactedUtils::GetIPCountry();
}

// Returns true if the image exists.
bool SteamUtils007::GetImageSize(int iImage, uint32 *pnWidth, uint32 *pnHeight)
{
	return RedactedUtils::GetImageSize(iImage, pnWidth, pnHeight);
}

// Returns true if the image exists.
bool SteamUtils007::GetImageRGBA(int iImage, uint8 *pubDest, int nDestBufferSize)
{
	return RedactedUtils::GetImageRGBA(iImage, pubDest, nDestBufferSize);
}

// Returns the IP of the reporting server for Valve.
bool SteamUtils007::GetCSERIPPort(uint32_t *unIP, uint16_t *usPort)
{
	return RedactedUtils::GetCSERIPPort(unIP, usPort);
}

// Return the percent of battery left, 255 for AC power.
uint8 SteamUtils007::GetCurrentBatteryPower()
{
	return RedactedUtils::GetCurrentBatteryPower();
}

// Returns the ID of the current process.
uint32 SteamUtils007::GetAppID()
{
	return RedactedUtils::GetAppID();
}

// Sets the position where the overlay instance for the currently calling game should show notifications.
void SteamUtils007::SetOverlayNotificationPosition(ENotificationPosition eNotificationPosition)
{
	return RedactedUtils::SetOverlayNotificationPosition(eNotificationPosition);
}

// API asynchronous call results
bool SteamUtils007::IsAPICallCompleted(SteamAPICall_t hSteamAPICall, bool *pbFailed)
{
	return RedactedUtils::IsAPICallCompleted(hSteamAPICall, pbFailed);
}
ESteamAPICallFailure SteamUtils007::GetAPICallFailureReason(SteamAPICall_t hSteamAPICall)
{
	return RedactedUtils::GetAPICallFailureReason(hSteamAPICall);
}
bool SteamUtils007::GetAPICallResult(SteamAPICall_t hSteamAPICall, void *pCallback, int cubCallback, int iCallbackExpected, bool *pbFailed)
{
	return RedactedUtils::GetAPICallResult(hSteamAPICall, pCallback, cubCallback, iCallbackExpected, pbFailed);
}
...
// Returns the number of IPC calls made since the last time this func was called.
// For performance debugging.
uint32 SteamUtils007::GetIPCCallCount()
{
	return RedactedUtils::GetIPCCallCount();
}

// API warning handling
void SteamUtils007::SetWarningMessageHook(SteamAPIWarningMessageHook_t pFunction)
{
	return RedactedUtils::SetWarningMessageHook(pFunction);
}

// Returns true if the overlay is enabled.
bool SteamUtils007::IsOverlayEnabled()
{
	return RedactedUtils::IsOverlayEnabled();
}

// Used when the game doesn't refresh in a loop.
bool SteamUtils007::BOverlayNeedsPresent()
{
	return RedactedUtils::BOverlayNeedsPresent();
}

// Creates a hash of the file.
SteamAPICall_t SteamUtils007::CheckFileSignature(const char *szFileName)
{
	return RedactedUtils::CheckFileSignature(szFileName);
}

// Gamepad support.
bool SteamUtils007::ShowGamepadTextInput(EGamepadTextInputMode eInputMode, EGamepadTextInputLineMode eInputLineMode, const char *szText, uint32 uMaxLength, const char * pUnk)
{
	return RedactedUtils::ShowGamepadTextInput(eInputMode, eInputLineMode, szText, uMaxLength, NULL);
}
uint32 SteamUtils007::GetEnteredGamepadTextLength()
{
	return RedactedUtils::GetEnteredGamepadTextLength();
}
bool SteamUtils007::GetEnteredGamepadTextInput(char *pchValue, uint32 cchValueMax)
{
	return RedactedUtils::GetEnteredGamepadTextInput(pchValue, cchValueMax);
}

const char *SteamUtils007::GetSteamUILanguage(){
	//noop
	return "EN";
}
bool SteamUtils007::IsSteamRunningInVR(){
	//noop
	return false;
}
